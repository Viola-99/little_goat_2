...
public class camera : MonoBehaviour
    ...
    void Start()
    {
        ...
        Camera.transform.position = new Vector3(
            (grid.nodes[0, 0].obj.transform.position.x + grid.nodes[grid.width - 1, grid.height - 1].obj.transform.position.x) / 2,
            8f,
            (grid.nodes[0, 0].obj.transform.position.z + grid.nodes[grid.width - 1, grid.height - 1].obj.transform.position.z) / 2
            );
        //}
    }
}
